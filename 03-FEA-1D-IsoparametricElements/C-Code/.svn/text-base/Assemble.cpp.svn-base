...
{ 
  ...
  for (int iel=0; iel<n_elem; iel++){
        ElemStiff( iel, elem_stiff, kel, pel, coords, elem_load, elem_node,nnode_ele, Ng, order, UMATid,DLOADid);
//	cout<<pel(0)<<"\t"<<pel(1)<<"\t"<<pel(2)<<"\n";
	GlobStiff( iel, kel, eq_num, elem_node, Kff, Kfp, Kpf, Kpp,pel, Pp, Pf, nnode_ele); 
  }
}
