			...
			, arma::colvec& coords, arma::colvec& elem_load, arma::imat& elem_node)
{
    ...
    kel(0,0) = elem_stiff(iel)/h;
    kel(1,1) = elem_stiff(iel)/h;
    kel(0,1) = elem_stiff(iel)*-1/h;
    kel(1,0) = elem_stiff(iel)*-1/h;

    pel = arma::ones<arma::colvec>(2);
    pel(0) = elem_load(iel)*0.5*h;
    pel(1) = elem_load(iel)*0.5*h;    
}
